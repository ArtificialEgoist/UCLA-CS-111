...
start(void)
{
	...
	// A process starts without holding up any other processes
	current->p_waiting_pid = -1;
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		// If exiting process is holding up another process, unblock it
			// by setting it to P_RUNNABLE
		if(current->p_waiting_pid > 0)
		{
			proc_array[current->p_waiting_pid].p_state = P_RUNNABLE;
			
			//Update system call return value on exit of process
	...
	case INT_SYS_WAIT: {
		...
		{
			// If process is in P_ZOMBIE state
				// Set it to P_EMPTY and return exit status
		...
		else
		{
			...
			// Block current process since we have to wait
				// and tell the new process the pid of
				// the current/waiting process
...
do_fork(process_t *parent)
{
	...
	// Traverse proc array from back to front
	// If any processes are empty, set emptyFound to true
	// Update firstEmpty to lowest index of empty process
	...
	{
		// Otherwise, create a new process_t
		// Update the child process's register values
		// Copy stack from parent, then return the new pid
		
		proc_array[firstEmpty].p_registers = parent->p_registers;
		proc_array[firstEmpty].p_registers.reg_eax = 0;
		
		copy_stack(&proc_array[firstEmpty], parent);
		...
		// Set its state to RUNNABLE, exit state to 0, and no waiting processes
		proc_array[firstEmpty].p_state = P_RUNNABLE;
		proc_array[firstEmpty].p_exit_status = 0;
		proc_array[firstEmpty].p_waiting_pid = -1;
		
		
		return proc_array[firstEmpty].p_pid;
	}
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// Top of a stack can be found by incrementing the base PROC1_STACK_ADDR
	// by PROC_STACK_SIZE multiplied by the process's id
	...
	// Copy everything from src_stack_bottom to dest_stack_bottom
	// For a total of (src_stack_top - src_stack_bottom) bytes
	memcpy((void *) dest_stack_bottom, (void *) src_stack_bottom, (src_stack_top - src_stack_bottom));
	
	// Set the new destination stack pointer to its bottom address
