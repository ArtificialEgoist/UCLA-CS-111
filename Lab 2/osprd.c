	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		*/
		...
		if (filp_writable) {	
			...
			if (pidInList(d->readLockingPids, current->pid) || pidInList(d->writeLockingPids, current->pid) || d->holdOtherLocks || filp->f_flags & F_OSPRD_LOCKED)
			{ 	
				if (d->holdOtherLocks){
					d->holdOtherLocks = 0;
				}
				printk("ENTERED FIRST WRITE IF");
				// UNLOCK	
				osp_spin_unlock(&(d->mutex));
				return -EBUSY;
			}
			...
			if (d->ticket_tail==myTicket && d->writeLockingPids == NULL && d->readLockingPids == NULL) { 
				...
				if (d->ticket_tail == myTicket) {
					grantTicketToNextAliveProcessInOrder(d); 
					printk("GRANTING NEXT TICKET\n");
				}
				else  {  
					addToTicketList(&(d->exitedTickets), myTicket);
					printk("ADDING TO EXITED TICKET\n");
				}
				return -ERESTARTSYS; 
			}
			else
			{
				printk("ELSE CLAUSE FOR WRITE\n");
				return -EBUSY;
			}
			...
			if (pidInList(d->writeLockingPids, current->pid) || d->holdOtherLocks || filp->f_flags & F_OSPRD_LOCKED) {
				if (d->holdOtherLocks) {
					d->holdOtherLocks = 0;
				}
				printk("ENTERED FIRST READ IF");
				// UNLOCK
				osp_spin_unlock(&(d->mutex));
				return -EBUSY;
			}
