 ...
 */
 ...
 // TASK 2B START: (2) set a constant to restrict the file size for downloading
	// The exact size shouldn't really matter and can be changed in the future
	#define MAXDOWNLOAD	134217728 // This is equivalent to 128 MB
// TASK 2B END 


// TASK 2B START: (3) increase robustness via buffer size to account for more peers
	// Define TASKBUFSIZ to be much larger than it currently is
	// The exact size shouldn't matter as long as it can adequately hold more processes
	//#define TASKBUFSIZ	4096	// Size of task_t::buf
	#define TASKBUFSIZ	73728
// TASK 2B END 
...
// TASK 2A START: prevent buffer overflow 
	// strcpy is notorious for buffer overflows when there's no null terminator
	// Instead, use strncpy, which fills up to specified length with extra zero bits
	// We know the max size of both buffers is FILENAMESIZ anyway, so fill up that much
	// We'll set the last null terminator manually just in case
...
// TASK 3 START: overflow other peer's buffer
	// As osp2p.h says about osp2p_writef(), we can keep adding more parameters to write into peer's buffer
	// If evil mode is on, we'll keep writing 4 times the filename until we can't write anymore
	// At that point, we try again for different peers with that download
	if(evil_mode!=0)
	{	
		write_more:
		// If the write was successful, we'll redirect back here and keep writing more
		if(osp2p_writef(t->peer_fd,"%s%s%s%s\n",t->filename, t->filename, t->filename, t->filename)==0)
			goto write_more;
		else
			goto try_again;
	}
...
// TASK 2B START: (2) check that the file's number of bytes is within max specified size
	//If it exceeds our max size, produce an error and retry
	if(t->total_written > MAXDOWNLOAD)
	{
		error("Download error: file exceeds maximum specified size!");
		goto try_again;
	}
// TASK 2B END
...
// TASK 2B START: (1) prevent peer from uploading files outside of current directory
	// Only files that don't contain a '/' character are contained in current directory
	// Otherwise, we'll need to return an error
	int notCurrDir=0;
	int i=0;
	// Go through all characters in filename
	// If null terminator, break out and continue
	// Otherwise, if we find a '/' character, set boolean value
	while(i<FILENAMESIZ)
	{
		if(t->filename[i]=='\0')
			break;
	...
	// If boolean value indicates file contains '/' character, it is not in current directory
	if(notCurrDir)
	{
		error("Upload error: file not found in current directory!");
		goto exit;
	}
// TASK 2B END
	...
	while (1) {
// TASK 3 START: stall the peer you are uploading to by looping write as long as we are in evil mode
	// In theory, the peer will be forever stuck as we keep writing junk into t->peer_fd
		while(evil_mode!=0)
			osp2p_writef(t->peer_fd, '\0');
// TASK 3 END
...
// TASK 1 START: parallel downloading/uploading of files
	
	// We can simply fork into different processes for downloading concurrently
	// Do all of this inside the loops for each file download or upload attempt
	
	 // ID of the child process that results from fork
	pid_t childPid; // Later, this will tell us whether we forked successfully in order to, say, download

	// First, download files named on command line.
	for (; argc > 1; argc--, argv++)
	{
		// We're about to start a download, so let's fork
		if ((t = start_download(tracker_task, argv[1])))
		{
			...
			if(childPid >= 0) // Successfully forked!
			{
				if(childPid == 0)
				{
					// Have child process download concurrently, then exit
					task_download(t, tracker_task);
					_exit(0);
				}
				else
				{
					// Have parent process do nothing
					// We are only using it to fork into more children for downloading
				}
			}
			else // Failed to fork!
			{
				error("Download error: failed to fork!");
			}
		}
	}
	...
	while ((t = task_listen(listen_task)))
	{
		...
		if(childPid >= 0) // Successfully forked!
		{
			if(childPid == 0)
			{
				// Have child process upload concurrently, then exit
				task_upload(t);
				_exit(0);
			}
			else
			{
				// Have parent process do nothing
				// We are only using it to fork into more children for uploading
			}
		}
		else // Failed to fork!
		{
			error("Upload error: failed to fork!");
		}
	}
